...
class CCCLoss(nn.Module):
    def __init__(self, digitize_num, range=[-1, 1], eps=1e-8):
        ...
        if self.digitize_num != 0:
            ...
            self.bins = Variable(torch.as_tensor(
                bins, dtype=torch.float32).cuda()).view((1, -1))
    ...
    def forward(self, x, y):
        ...
        rho = torch.sum(vx * vy) / (torch.sqrt(torch.sum(torch.pow(vx, 2)))
                                    * torch.sqrt(torch.sum(torch.pow(vy, 2)))+self.eps)
        x_m = torch.mean(x)
        y_m = torch.mean(y)
        x_s = torch.std(x)
        y_s = torch.std(y)
        ccc = 2*rho*x_s*y_s/(torch.pow(x_s, 2) +
                             torch.pow(y_s, 2) + torch.pow(x_m - y_m, 2))
        return 1-ccc
